...
namespace Fnio.Lib.HtmlQuery
{
    static class HtmlParser
    {
        ...
        public static HtmlDocument Parse(string html, IEnumerable<string> withoutTags)
        {
            html = TrimHtml(html);
            ...
            var tagSet = withoutTags != null
                ? new HashSet<string>(withoutTags.Select(s => s.ToLowerInvariant()))
                : null;
        ...
        private static HtmlNode ToHtmlNode(this XNode x, HtmlDocument doc, HashSet<string> withoutTags)
        {
            if (x.NodeType == XmlNodeType.Element && x is XElement)
                return ToHtmlElement(x as XElement, doc, withoutTags);
        ...
        private static HtmlElement ToHtmlElement(this XElement x, HtmlDocument doc, HashSet<string> withoutTags)
        {
            var tag = x.Name.LocalName.ToLowerInvariant();

            if (withoutTags != null && withoutTags.Contains(tag))
                return null;

            var attrs = x.GetHtmlAttributes();
            var childNodes = x.Nodes().Select(n => n.ToHtmlNode(doc, withoutTags)).Where(n => n != null);
